<template>
  <el-container>
    <el-header class="cate_mana_header">
      <el-input placeholder="请输入公告内容" v-model="message" style="width: 200px;"></el-input>
    ...
    <el-main class="cate_mana_main">
      ...
      >
        ...
        <el-table-column label="公告内容" prop="message" width="120" align="left"></el-table-column>
        <el-table-column label="最近编辑时间" width="140" align="left">
          <template slot-scope="scope">{{ scope.row.publishDate | formatDateTime}}</template>
        </el-table-column>
        <el-table-column label="发布人" prop="nickname" width="120" align="left"></el-table-column>
        <el-table-column label="删除时间" width="140" align="left">
          <template slot-scope="scope">{{ scope.row.deleteTime | formatDateTime}}</template>
        </el-table-column>
        <el-table-column label="删除记录的人" prop="dename" width="120" align="left"></el-table-column>
...
export default {
  methods: {
    add() {
      ...
      postRequest("/admin/notice/add", { message: this.message, uid: id }).then(
        resp => {
          if (resp.status == 200) {
            var json = resp.data;
    ...
    handleEdit(index, row) {
      ...
      this.$prompt("请输入新名称", "编辑", {
        ...
        if (value == null || value.length == 0) {
          ...
          _this.loading = true;
          putRequest("/admin/notice/", { id: row.id, message: value }).then(
            resp => {
              var json = resp.data;
              _this.$message({
                type: json.status,
    ...
    deleteCate(ids) {
      ...
      _this.$alert(ids);
      //删除
      putRequest("/admin/notice/delete/" + ids).then(
        resp => {
          var json = resp.data;
          _this.$message({
            type: json.status,
    ...
    refresh() {
      let _this = this;
      getRequest("/admin/notice/all").then(
        resp => {
