<template>
  <el-row v-loading="loading">
    ...
    <el-col :span="24">
      <div>
        <div>
          <h3 style="margin-top: 0px;margin-bottom: 0px" v-model="notice.title">{{notice.title}}</h3>
        </div>
        <div
        ...
        >
          <div
            ...
            v-model="notice.nickname"
          >{{notice.nickname}}</div>
          <span
            style="color: #20a0ff;margin-right:20px;font-size: 12px;"
          >{{notice.publishDate | formatDateTime}}</span>
        </div>
      </div>
    </el-col>
    <el-col>
      <div style="text-align: left" v-html="notice.htmlContent"></div>
    </el-col>
  </el-row>
</template>
<script>
