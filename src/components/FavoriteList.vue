...
<!--    </el-header>-->
    ...
    <el-main class="cate_mana_main">
      ...
      <el-button type="danger" :disabled="this.selItems.length==0" style="margin-top: 10px;width: 100px;"
                 @click="deleteAll" v-if="this.favorite.length>0">批量删除
      </el-button>
    </el-main>
  </el-container>
</template>
    ...
    export default{
        methods: {
            addNewCate(){
                this.loading = true;
                var _this = this;
                postRequest('/admin/category/regist', {title: this.title}).then(resp=> {
            ...
            handleEdit(index, row){
                ...
                this.$prompt('请输入新名称', '编辑', {
                    ...
                    if (value == null || value.length == 0) {
                        ...
                        _this.loading = true;
                        putRequest("/admin/category/", {id: row.id, title: value}).then(resp=> {
                            var json = resp.data;
                            _this.$message({
                                type: json.status,
                                message: json.msg
