  ...
  <el-container>
    ...
    <el-main class="cate_mana_main">
      ...
      >
        ...
        <el-table-column label="栏目名称" prop="title" width="120" align="left"></el-table-column>
...
export default {
  methods: {
    addNewCate() {
      this.loading = true;
      var _this = this;
      postRequest("/admin/category/regist", { title: this.title }).then(
        resp => {
          if (resp.status == 200) {
            var json = resp.data;
    ...
    handleEdit(index, row) {
      ...
      this.$prompt("请输入新名称", "编辑", {
        ...
        if (value == null || value.length == 0) {
          ...
          _this.loading = true;
          putRequest("/admin/category/", { id: row.id, title: value }).then(
            resp => {
              var json = resp.data;
              _this.$message({
                type: json.status,
