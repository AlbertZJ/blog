...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="文章标题" width="400" align="left">
        <template slot-scope="scope">
          <span
            ...
            @click="itemClicks(scope.row)"
          >{{ scope.row.title}}</span>
        </template>
      </el-table-column>
      <el-table-column label="评论内容" width="400" align="left">
        <template slot-scope="scope">
          <span
            style="color: #409eff;cursor: pointer"
      ...
      <el-table-column label="发布时间" width="140" align="left">
        <template slot-scope="scope">{{ scope.row.publishDate | formatDateTime}}</template>
      </el-table-column>
      <el-table-column prop="nickname" label="谁评论" width="120" align="left"></el-table-column>
    ...
    <div class="blog_table_footer">
      <el-button
        ...
        size="mini"
        style="margin: 0px;"
        v-show="this.comment.length>0 && showDelete"
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      this.$router.push({
        path: "/editComment",
        query: { from: this.commentName, id: row.id }
      });
    },
