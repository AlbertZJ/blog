      ...
      <el-card style="margin-top: 10px;" v-for="(comment,index) in comment" :key="index"
        ...
        <div slot="header" style="text-align: left">
          <div><img :src="comment.userface" :alt="comment.nickname" style="width: 70px;height: 70px"> <span>{{comment.comname}}</span>
            <div style="display: flex;justify-content: flex-start">
              <el-button type="text" size="mini" style="margin-left: 3px;color:black" @click="comlike(comment.id)">
                喜欢{{comment.countlike}}
              </el-button>
          ...
          <div>
            ...
            <el-button size="medium" style="margin-left: 10px" @click="addCom(comment.id)" v-if="comment.parentId==-1">
              评论
            </el-button>
            <el-button size="medium" style="margin-left: 10px" @click="deleteCom(comment.id)"
                       v-if="comment.uid==UserId">删除
            </el-button>
          </div>
        </div>
      </el-card>
